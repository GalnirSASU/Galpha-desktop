...
export function ApiKeySetup({ onApiKeySet, onSkip }: ApiKeySetupProps) {
  ...
  const handleSave = async () => {
    ...
    if (!apiKey.startsWith('RGAPI-')) {
      setError('Format de clé invalide. Les clés API Riot commencent par "RGAPI-"');
      return;
    }
    ...
    <div className="min-h-screen tiled-background flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-base-darker/80 backdrop-blur-sm rounded-2xl shadow-2xl p-8 border border-base-light/20">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Configuration requise</h1>
          <p className="text-gray-400">Entrez votre clé API Riot Games pour continuer</p>
        </div>

        {/* Instructions */}
        <div className="bg-base-dark/50 rounded-xl p-4 mb-6 border border-base-light/10">
          <h3 className="text-sm font-semibold text-white mb-2">Comment obtenir votre clé API:</h3>
          <ol className="text-sm text-gray-400 space-y-1 list-decimal list-inside">
            <li>Visitez <a href="https://developer.riotgames.com/" target="_blank" rel="noopener noreferrer" className="text-accent-primary hover:text-accent-secondary transition-colors">developer.riotgames.com</a></li>
            <li>Connectez-vous avec votre compte Riot</li>
            <li>Copiez votre "Development API Key"</li>
            <li>Collez-la ci-dessous</li>
          </ol>
          <p className="text-xs text-gray-500 mt-2">
            Note: Les clés de développement expirent après 24 heures
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          >
            {saving ? 'Sauvegarde...' : 'Enregistrer la clé API'}
          </button>
          ...
          {onSkip && (
            <button
              onClick={onSkip}
              disabled={saving}
              className="w-full bg-base-dark/50 border border-base-light/30 text-gray-300 hover:text-white font-medium py-3 px-6 rounded-lg hover:bg-base-light/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
